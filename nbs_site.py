...
/* height:auto matters because the figures now carry real width and height
   attributes, which reserve the right box while the image loads. Without it
   the attribute height is taken literally and a 2000x1000 shot scaled to
   1016px wide is still drawn 1000px tall. */
...
@media(prefers-reduced-motion:no-preference){
  /* Gated on html.js, which the one-line script in <head> adds. A .reveal
     element starts invisible and is shown when the observer adds .in - so if
     the script never runs, hiding it is a promise nothing will keep.
     On the exported site nothing does run: vercel.json sends
     script-src 'none', which blocked every script on the page and left all
     nine reveals on the homepage, and all six screenshots, permanently at
     opacity 0. The site looked like it had lost its images.
     No script, no class, nothing hidden. The animation is decoration; the
     content is not. */
...
def _record_figs(record):
    ...
    figs = [
        ("6,158", "trades on the current rules over three years"),
        ("PF 1.05", "profit factor in the held-out year, after costs: slightly positive"),
        ("\u20b94.81 lakh", "worst drawdown per lot in that year"),
 ...
 <section>
  <div class="callout warm reveal">
   ...
   <p>This is the part a site like this normally leaves out, so it is on the
    ...
    they came out <b>positive after costs in the first two years and slightly
    positive in the held-out final year</b> &mdash; Nifty and Sensex made
    money in both, and Bank Nifty lost about half of what they made in the final
 ...
 <section>
  ...
  <table class="tbl">
   ...
   <tr><td>CONFIRMING</td><td>the direction has to hold for
    ...
    issued</td></tr>
   <tr><td>COOLDOWN</td><td>a second ticket the same way on an index waits
    <code>{_cfg("REENTRY_COOLDOWN_MIN", 20)}</code> minutes after its last entry
    or exit, so a stop-out is not bought straight back. A gap between any two
    tickets can also be set, currently
    <code>{_cfg("MIN_MINUTES_BETWEEN_TICKETS", 0)}</code> minutes.
    <b>Skip cooldown</b> on the signal card lifts the wait once &mdash; every
    other check still applies, and the ticket is marked in the trade
    log</td></tr>
...
<div class="wrap">
 ...
 <section>
  <div class="narrow prose">
   ...
   <p>On a computer the sections sit in a rail down the left: Home and the
    Signal, then the chart, the option chain, the market map, market pulse,
    sector scope and momentum spikes, the analysis tabs, news and your record.
    Your account and the date your access renews are at the foot of the rail.
    On a phone the rail is a drawer behind the menu button at the top, and
    Home, Signal, Chart and the option chain sit in a bar along the bottom
    within reach of a thumb.</p>
   ...
   <table class="tbl">
    ...
    <tr><td>Reward : risk</td><td>how far the market has room to run, divided
     by the distance to the stop. Below the floor it never became a ticket at
     all. It is not what the exit pays &mdash; that is in the Risk and reward
     panel.</td></tr>
    ...
    <tr><td>The percentage beside each level</td><td>the chance of the index
     touching it before the 15:30 close (on Bitcoin, within the next 24 hours),
     from the option chain&rsquo;s implied volatility and calibrated against
     past trades. The chances overlap &mdash; a trade can touch a target and
     still hit the stop &mdash; so they do not add up to 100 and are not a win
     rate.</td></tr>
    <tr><td>Risk and reward</td><td>under the ladder: what the stop costs and
     what each target makes, in rupees at your lot count after Zerodha&rsquo;s
     charges, how many times the risk each target is, and the win rate the
     trade needs to break even.</td></tr>
    <tr><td>Skip cooldown</td><td>appears while a cooldown is the only thing
     holding a ticket. It lifts the wait once; every other check still has to
     pass, and a ticket issued that way is marked in the trade log.</td></tr>
    <tr><td>Holding overnight</td><td>in the last hour, an open ticket shows
     what carrying the position past the close would cost in time decay
     before the next session &mdash; three nights over a weekend &mdash; and
     what buying into the close did in a three-year test. The tool closes its
     own ticket at the bell.</td></tr>
    <tr><td>What if</td><td>on an open Nifty, Bank Nifty or Sensex ticket: the
     premium and the rupee change if the index moves 0.5% either way, if
     volatility falls 2 points, a day later with nothing moving, and with all
     three against the position at once. Repriced in full with the same model
     as the chain rather than estimated from delta and vega &mdash; an
     estimate, not a quote.</td></tr>
   ...
   <table class="tbl">
    ...
    <tr><td>Screener</td><td>Build your own screen over the Nifty, Bank Nifty and
     Sensex member stocks: RSI, CCI, Williams %R, moving averages (SMA, EMA, WMA),
     MACD, ADX, ATR, Parabolic SAR, Bollinger Bands and volume, on daily, weekly or monthly candles, compared with a
     number or with another indicator, now or a set number of bars ago &mdash;
     including a line crossing another. Start from a preset, change it, save your own.
     It shows which stocks match right now and the values that decided it; it does
     not say whether trading them makes money.</td></tr>
...
<div class="wrap"><div class="narrow prose">
 ...
 <section>
  ...
  <ul>
   ...
   <li><b>All of the held-out year&rsquo;s profit came on expiry days.</b>
    ...
    &#8377;3,53,503 per lot in that year (379 trades, profit factor 2.06); every
    other day together <b>lost &#8377;2,09,475</b> (2,141 trades, 0.92). Expiry
   ...
   <li><b>The held-out year is no longer clean.</b> Several of today&rsquo;s
    ...
    RSI divergence, and on 16 Sep giving a trend day more room to run. A test you have used to
   ...
   <li><b>The drawdowns are large.</b> The worst run in the held-out year was
    &#8377;4,80,832 per lot, with the three indices&rsquo; trades taken in the
   ...
   <li><b>One input was measured inside the held-out year.</b> The VWAP vote
    ...
    measurement at all &mdash; the held-out year made &#8377;14,044 less and the
 ...
 <section>
  ...
  <table class="tbl">
   ...
   <tr><td>First two years <small>(to 15 Aug 2025)</small></td><td>3,638</td><td>+&#8377;6,29,112</td><td>+&#8377;173</td><td>1.16</td><td>&#8377;1,74,098</td></tr>
   <tr><td>Held-out year <small>(15 Aug 2025 to 11 Sep 2026)</small></td><td>2,520</td><td>+&#8377;1,44,028</td><td>+&#8377;57</td><td>1.05</td><td>&#8377;4,80,832</td></tr>
  </table>
  <p>A profit factor of 1.05 means about &#8377;1.05 won for every &#8377;1 lost:
  ...
  <table class="tbl">
   ...
   <tr><td>Nifty</td><td>+&#8377;3,76,779</td><td>1.32</td><td>+&#8377;1,25,558</td><td>1.13</td></tr>
   <tr><td>Bank Nifty</td><td>+&#8377;19,143</td><td>1.01</td><td>&minus;&#8377;1,36,307</td><td>0.87</td></tr>
   <tr><td>Sensex</td><td>+&#8377;2,33,189</td><td>1.20</td><td>+&#8377;1,54,777</td><td>1.19</td></tr>
  </table>
 ...
 <section>
  ...
  <ul>
   ...
   <li><b>More room to run on a trend day.</b> The room a trade needs is what is
...
<div class="wrap"><div class="narrow prose">
 <section class="first">
  ...
  <p>The current rules were backtested across three years of 15-minute candles
   ...
   slightly positive in the held-out year (Bank Nifty lost; Nifty and Sensex
